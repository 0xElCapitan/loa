...
**Recurrence count**: ≥6 (documented Attempts rows; true count higher — many: every cycle since the bug landed, ~every sprint hits it; normalized cycle-121)
...
| 2026-07-29 | br create --parent (br 0.2.6) during cycle-121 kickoff | FAILED: 'no such column: last_child in table child_counters'. Routed around per Reading guide: beads created flat with cycle/sprint labels, no parent-child deps. | run-20260729-68d1b75c |
...
**Recurrence count**: ≥5 (documented Attempts rows; true count higher — every PR since `dd54fe9c` that touches `model-config.yaml` or related paths; workflow only triggers on PRs, not main pushes — so main is "passing" by virtue of not running; normalized cycle-121)
...
**Recurrence count**: 1 (resolved in same session as discovery, cycle-102 sprint-1E; normalized cycle-121)
